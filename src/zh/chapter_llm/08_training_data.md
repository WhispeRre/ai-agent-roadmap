...
**练习 1（概念）**：本章反复强调 SFT 阶段"少而精"，LIMA 论文用 1000 条数据就能逼近顶级对话质量。请解释：为什么 SFT 不需要海量数据？如果有人坚持"数据越多越好"，收集了 10 万条未审核的数据，可能带来什么坏处？

<details>
<summary>参考答案</summary>

**为什么少量就够：** SFT 的本质不是教模型"新知识"，而是教**行为格式**。预训练阶段模型已经从海量文本里学到了丰富的世界知识和语言能力，它"什么都懂，只是不知道该用什么格式、什么风格来回答你"。SFT 做的事更像"教礼仪"——教一个本来就博学的人如何得体地对话，自然不需要上万堂课。从概率角度看，SFT 只是在微调输出分布的"形状"（格式、风格），不需要改变分布的"核心内容"（知识）。

**海量低质数据的坏处：**
- **错误被直接学进去**：SFT 是模仿学习，数据里的事实错误、格式错误会被模型当成"正确示范"照单全收（垃圾进垃圾出）。
- **高质量信号被稀释**：1000 条精品里混进 9 万条平庸样本，模型学到的平均水准反而被拉低。
- **灾难性遗忘风险**：过度微调会侵蚀预训练学到的通用能力。
- **成本浪费**：花了大量标注资源，效果却往往不如 1000 条经人工验证的数据。

这就是本章说的"最违反直觉但最重要的规律"：质量 >> 数量。

</details>

**练习 2（辨析）**：判断下面这句话对不对并说明理由——"RL 训练和 SFT 一样，问题越多、越难越好，所以应该多收集那些模型一道都做不出来的超难题来逼它进步。" 请结合"有效奖励区间"的概念分析。
...
**第一，RL 数据不是越多越好，而看"有效奖励区间"。** RL 通过对比同一问题的多个采样回答（有好有坏）来学习。一道题如果模型 **100% 做对**，所有采样奖励都是 1，组内标准化后优势全为 0，学不到东西；如果 **0% 做对**，所有采样奖励都是 0，同样没有区分度。只有模型**通过率在 30%—70%** 的"刚好够难"的题，采样后才有好有坏，形成有效的对比信号。

**第二，"超难题越多越好"恰恰是反例。** 模型一道都做不出来的题，通过率是 0%，属于"完全没有价值"的数据。

类比考试出题：太简单的题全班满分、太难的题全班零分，两种都无法区分学生水平；最有价值的是"有区分度的中等难度题"。所以正确做法是做**难度校准**，筛掉太简单和太难的，保留落在模型"学习区"里的问题。注意这和 SFT 逻辑完全不同——SFT 要的是高质量标准答案，RL 要的是"难度刚好"的问题 + 好的奖励函数。

</details>

**练习 3（动手）**：你要为一个"数学解题 Agent"做 RL 训练，需要一个奖励函数。请写出一个规则奖励函数，要求做到**多维度打分**（不能只看最终答案对错），以此降低 Reward Hacking 的风险。说明你为什么这样设计。
...
def math_reward(response: str, ground_truth: str) -> float:
    """
    多维度数学题奖励，总分范围约 [0, 1.2]
    设计目标：不让模型靠"猜答案"或"堆字数"作弊
    """
    reward = 0.0

    # 维度 1：答案格式正确（必须放进 \boxed{}）—— +0.1
    boxed = re.search(r"\\boxed\{(.+?)\}", response)
    if boxed:
        reward += 0.1
        predicted = boxed.group(1).strip()

        # 维度 2：答案正确（支持数值容差）—— +1.0
        try:
            if abs(float(predicted) - float(ground_truth)) < 1e-6:
                reward += 1.0
        except ValueError:
...
- **只看答案对错会被"钻空子"**：如果奖励函数只检查最终数字，模型会学会跳过推理直接猜答案——这就是典型的 Reward Hacking。
- **加入格式分（`\boxed{}`）**：强制答案出现在固定位置，便于自动提取，也让输出更规范。
- **加入推理过程分**：鼓励模型"想清楚再答"，而不是蒙。注意这里要求 think 内容**有一定长度**，否则模型会写个空 `<think></think>` 来骗分——这是给作弊行为又加了一道防线。
- **多维度组合**：本章指出防范 Reward Hacking 的关键就是"奖励函数要多维度（正确性 + 格式 + 推理质量）"。单一维度太容易被攻破。

进阶可补充：奖励长度时设上限（防止凑字数）、定期人工抽查训练输出、用 KL 散度约束限制策略偏移（PPO/GRPO 已内置）。
